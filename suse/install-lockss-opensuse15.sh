...
# directory
cat > /etc/zypp/repos.d/lockss.repo <<'EOF'
[lockss] 
name = LOCKSS Daemon Repository 
baseurl=http://www.lockss.org/repo/ 
gpgcheck=1
EOF
chmod 644 /etc/zypp/repos.d/lockss.repo

# Import the LOCKSS GPG key
rpm --import http://www.lockss.org/LOCKSS-GPG-RPM-KEY

# Allow unsigned repomd.xml
zypper -n mr --gpgcheck-allow-unsigned-repo http://www.lockss.org/repo/ "LOCKSS Daemon Repository"

# Zypper imports the LOCKSS GPG key
zypper -n --gpg-auto-import-keys ref lockss
...
#   dstat - dstat command
#   git - git command
#   insserv-compat - enable System V init scripts in SUSE
#   iotop - iotop command
#   lshw - lshw command
#   lsof - lsof command
#   mailx - mail command
...
#   wget - wget command
zypper -n install lockss-daemon java-1_8_0-openjdk dstat git insserv-compat iotop lshw nmap pciutils sysstat

# enable LOCKSS to start on boot
# insserv lockss
chkconfig lockss on
